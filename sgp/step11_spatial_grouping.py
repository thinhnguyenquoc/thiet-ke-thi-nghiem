...
def main():
    print(f"🚀 Step 11: Spatial Grouping Experiment (Singapore) - {N_CLUSTERS} Clusters...")
    ...
    plt.title(f'Singapore Subzones Divided into {N_CLUSTERS} Spatial Clusters')
    plt.savefig(PLOT_FILE)
    print(f"📈 Cluster map saved to {PLOT_FILE}")

    # 3. Preparation
    subzone_ids = gdf['SUBZONE_C'].values
    merged_poi = gdf.merge(poi_df[['SUBZONE_C', 'A_j']], on='SUBZONE_C', how='left').fillna(0)
    aj_values = merged_poi['A_j'].values
    ...
    real_trips['DEST_SUBZONE'] = real_trips['DEST_SUBZONE'].astype(str)
    ...
    for cluster_id in range(N_CLUSTERS):
        print(f"--- Fold {cluster_id + 1}/{N_CLUSTERS} (Cluster {cluster_id}) ---")
        ...
        print(f"✅ Cluster {cluster_id} -> Avg CPC: {cpc:.4f}")
