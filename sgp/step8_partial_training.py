...
def calculate_global_cpc(actual_df, pred_matrix, all_zones):
    ...
    for _, row in actual_df.iterrows():
        o, d, count = str(row['ORIGIN_SUBZONE']), str(row['DESTINATION_SUBZONE']), row['COUNT']
...
def main():
    print("🚀 Step 8: Partial-Training Shell Experiment (Singapore - Random Selection)...")
    ...
    binned_df['ORIGIN_SUBZONE'] = binned_df['ORIGIN_SUBZONE'].astype(str).str.strip().str.replace('.0', '', regex=False)
    
    print("📖 Loading raw trips for CPC validation...")
    raw_df = pd.read_csv(RAW_TRIPS)
    raw_df['ORIGIN_SUBZONE'] = raw_df['ORIGIN_SUBZONE'].astype(str).str.strip().str.replace('.0', '', regex=False)
    raw_df['DESTINATION_SUBZONE'] = raw_df['DESTINATION_SUBZONE'].astype(str).str.strip().str.replace('.0', '', regex=False)
    
    poi_df = pd.read_csv(POI_FILE)
    poi_df['SUBZONE_C'] = poi_df['SUBZONE_C'].astype(str).str.strip().str.replace('.0', '', regex=False)
    ...
    plt.title('Mobility Scale Robustness (Singapore)', fontsize=14)
