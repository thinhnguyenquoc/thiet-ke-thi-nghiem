...
3.  **Exponential Decay**: Mô hình Gravity parametric sử dụng hàm suy giảm mũ.
    $$ \hat{T}_{ij} = \frac{m_i^\alpha n_j^\beta}{\exp(\gamma r_{ij})} $$
4.  **Power Decay**: Mô hình Gravity parametric sử dụng hàm suy giảm lũy thừa.
    $$ \hat{T}_{ij} = \frac{m_i^\alpha n_j^\beta}{r_{ij}^\gamma} $$
