...
Nghiên cứu thực hiện so sánh đối chiếu 6 biến thể mô hình để đánh giá ảnh hưởng của cấu trúc không gian và dữ liệu tiện ích:

1.  **Radiation (Pop)**: Mô hình bức xạ truyền thống sử dụng quy mô dân số và cơ hội xen giữa để dự báo.
2.  **Radiation (POI)**: Biến thể mô hình bức xạ sử dụng tổng số lượng POI làm "khối lượng hấp dẫn" thay cho dân số.
3.  **Exponential Decay**: Mô hình Gravity parametric sử dụng hàm suy giảm mũ $e^{-\gamma r}$.
4.  **Power Decay**: Mô hình Gravity parametric sử dụng hàm suy giảm lũy thừa $r^{-\gamma}$.
5.  **Attraction-Uniform**: Mô hình vỏ (Shell) 1km đề xuất, phân bổ đều luồng di chuyển cho các vùng đích trong cùng dải khoảng cách.
6.  **Attraction-Weighted**: Mô hình tối ưu đề xuất, phân bổ luồng di chuyển trong dải khoảng cách dựa trên trọng số hấp dẫn POI.
...
- **Ưu thế của cấu trúc vỏ (Shells)**: Việc sử dụng các dải khoảng cách thực nghiệm (1km) giúp tăng độ chính xác lên gần **100%** (từ ~0.30 lên >0.60 CPC) tại cả hai thành phố.
- **Giá trị của dữ liệu POI**: Tích hợp dữ liệu POI giúp giảm sai số MAE tại Seoul từ 1.387 xuống còn **1.154** (giảm **16.7%**).
- **Cải thiện mô hình Radiation**: Việc thay thế dân số bằng POI giúp cải thiện CPC tại Singapore từ **0.18** lên **0.27**.
...
- **So sánh giữa Singapore và Seoul**: Mặc dù Singapore có diện tích nhỏ hơn và cấu trúc nén hơn, mô hình đề xuất vẫn duy trì tính ổn định cao. Chỉ số CPC tại Seoul cao hơn (0.76 so với 0.67) có thể do dữ liệu OD của Seoul có độ phân giải về lưu lượng tập trung hơn, giúp mô hình trọng số POI phát huy tối đa hiệu quả.
