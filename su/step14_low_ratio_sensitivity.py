...
OUTPUT_FILE = os.path.join(CURRENT_DIR, 'step14_subzone_sensitivity_results.csv')
CITY_CRS = 'EPSG:5179'
N_CLUSTERS = 10
RATIOS = [0.09, 0.08, 0.07, 0.06, 0.05, 0.04, 0.03, 0.02, 0.01]
N_ITERATIONS = 20
...
def main():
    print(f"🚀 Step 14: Low-Ratio Subzone Sensitivity (Seoul)...")
    print(f"Ratios: {RATIOS}, Iterations: {N_ITERATIONS}")
    ...
    # 3. Sensitivity Loop (Distributed Random Sampling)
    results_record = []
    ...
    for ratio in RATIOS:
        ...
        print(f"📊 Ratio {ratio*100:.1f}% -> Avg Global CPC: {avg_cpc:.4f} (std: {std_cpc:.4f})")
    ...
    plt.plot(results_df['Ratio'] * 100, results_df['Avg_CPC'], marker='o', color='purple', label='Distributed Sample')
    plt.xlabel('Subzone Sample Ratio (%)')
    plt.ylabel('Global CPC Score')
    plt.title('Seoul Mobility Law: Sensitivity in Ultra-Low Data Regime (1%-9%)')
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.savefig(os.path.join(CURRENT_DIR, 'step14_low_ratio_curve.png'))
    print(f"📈 Plot saved to su/step14_low_ratio_curve.png")
