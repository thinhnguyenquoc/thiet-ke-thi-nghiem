...
def main():
    ...
    # Need Pop and Centroids for parametric models
    ...
    with Image.open(POP_TIF) as img:
        ...
        mask = (pop_data > 0)
        pixel_gdf = gpd.GeoDataFrame({'pop': pop_data[mask]}, geometry=[Point(x, y) for y in lats for x in lons if pop_data[np.where(lats==y)[0][0], np.where(lons==x)[0][0]] > 0], crs="EPSG:4326")
        # Optimization: Since we did this before, I'll just use the already mapped pop if I had it.
        # But for correctness, I'll join again.
    ...
    for count, origin in enumerate(unique_origins):
        ...
        # 5-6: Calculate Parametric on the fly
