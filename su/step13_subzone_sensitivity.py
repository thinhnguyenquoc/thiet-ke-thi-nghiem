...
def main():
    print(f"🚀 Step 13: Subzone Sensitivity Experiment (Seoul)...")
    print(f"Ratios: {RATIOS}, Iterations: {N_ITERATIONS} per group")
    ...
    # Precompute spatial structures for ALL origins
    print("📏 Precomputing spatial bins for all origins...")
    ...
    # 3. Sensitivity Loop (Simultaneous Sampling from all Groups)
    results_record = []
    
    for ratio in RATIOS:
        print(f"\n--- Analyzing Sensitivity with {int(ratio*100)}% Sampling from EVERY Group ---")
        iter_cpcs = []
        
        for i in range(N_ITERATIONS):
            # Sample R% from EACH group and combine
            all_sampled_zones = []
            for cluster_id in range(N_CLUSTERS):
                group_zones = list(gdf[gdf['cluster'] == cluster_id]['SUBZONE_C'].values)
                num_to_sample = max(1, int(len(group_zones) * ratio))
                all_sampled_zones.extend(random.sample(group_zones, num_to_sample))
            
            # Calculate Global Law from combined subset
            ...
            # Predict for ALL origins in Seoul
            all_pred = []
            for origin_zone in unique_origins:
            ...
            print(f"  Iteration {i+1}/20 -> CPC: {cpc:.4f}")
        
        avg_cpc = np.mean(iter_cpcs)
        std_cpc = np.std(iter_cpcs)
        print(f"📊 Ratio {int(ratio*100)}% -> Average Global CPC: {avg_cpc:.4f} (std: {std_cpc:.4f})")
        results_record.append({'Ratio': ratio, 'Avg_CPC': avg_cpc, 'Std_CPC': std_cpc})
    ...
    plt.plot(results_df['Ratio'] * 100, results_df['Avg_CPC'], marker='o', linestyle='-', color='r', label='Sampled Global Law')
    plt.axhline(y=0.7623, color='blue', linestyle='--', label='Full Data Baseline (0.7623)')
    plt.xlabel('Subzone Sample Ratio (%)')
    plt.ylabel('Global CPC Score')
    plt.title('Seoul Mobility Law: Sensitivity to Distributed Subzone Sampling')
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.legend()
    plt.savefig(os.path.join(CURRENT_DIR, 'step13_sensitivity_curve.png'))
    print(f"📈 Sensitivity curve saved to su/step13_sensitivity_curve.png")
