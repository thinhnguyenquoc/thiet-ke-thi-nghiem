...
def main():
    print("🚀 Step 7: Production-Constrained Gravity model (Pop vs POI)...")
    ...
    if os.path.exists(POI_FILE):
        ...
        gdf_utm = gdf_utm.reset_index().merge(poi_df[['SUBZONE_C', 'A_j']], on='SUBZONE_C', how='left').fillna(0).set_index('SUBZONE_C')
    else:
        print("⚠️ POI_FILE not found, using POI attraction = 1 for all zones")
        gdf_utm['A_j'] = 1.0
    
    # 4. Load Trips and calculate O_i
    ...
    print(f"\n📊 FINAL COMPARISON SAVED TO {OUTPUT_FILE}")
    print(results_df)
