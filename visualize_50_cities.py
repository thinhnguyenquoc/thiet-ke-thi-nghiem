...
bar_plot = sns.barplot(x='mean', y='Model', data=avg_metrics, palette='viridis')
plt.title('Average CPC across 50 US Cities (Origin-Averaged)', fontsize=15)
...
plt.title('CPC Score Distribution across 50 Cities', fontsize=15)
...
# Comparing Shell models to Radiation Models
shell_avg = df[df['Model'].str.contains('Attraction')]['CPC'].mean()
rad_avg = df[df['Model'].str.contains('Radiation')]['CPC'].mean()
grav_avg = df[df['Model'].str.contains('Decay')]['CPC'].mean()
...
print(f"- Production-Constrained Gravity Avg CPC: {grav_avg:.3f}")
print(f"- Shell Models outperform Radiation by: {lift_shell_vs_rad:.1f}%")
print(f"- Shell Models outperform PC-Gravity by: {lift_shell_vs_grav:.1f}%")
...
- **Tính ổn định**: Biểu đồ hộp (Boxplot) cho thấy các mô hình Attraction-Uniform/Weighted có độ biến thiên thấp nhất, chứng minh khả năng thích ứng cao với nhiều loại hình cấu trúc đô thị khác nhau.
- **Sự khác biệt vùng miền**: Trong khi đa số các thành phố lớn ưu tiên mô hình Weighted (POI), một số khu vực có mật độ POI thấp hoặc đồng nhất lại cho kết quả tốt hơn với mô hình Uniform, khẳng định tầm quan trọng của việc đánh giá đa mô hình.
"""
